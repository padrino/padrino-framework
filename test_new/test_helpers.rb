...
describe Padrino::Helpers do
  ...
  it 'Base.mime_type registers mime type' do
    mock_app {
  ...
  describe 'attachment' do
    def attachment_app(filename=nil)
      mock_app {       
        get '/attachment' do
          attachment filename
          response.write("<padrino></padrino>")
        end
      }
    end
    ...
    it 'sets the Content-Type response header' do
      ...
      assert_equal '<padrino></padrino>', body
    end
    ...
    [Time.now, DateTime.now, Date.today, Time.now.to_i,
      ...
      describe "with #{last_modified_time.class.name}" do
        before do
          ...
          wrapper = Object.new.extend Padrino::Helpers
        ...
        describe "when there's no If-Modified-Since header" do
        ...
        describe "when there's an invalid If-Modified-Since header" do
          it 'sets the Last-Modified header to a valid RFC 2616 date value' do
        ...
        describe "when the resource has been modified since the If-Modified-Since header date" do
          it 'sets the Last-Modified header to a valid RFC 2616 date value' do
        ...
        describe "when the resource has been modified on the exact If-Modified-Since header date" do
          it 'sets the Last-Modified header to a valid RFC 2616 date value' do
        ...
        describe "when the resource hasn't been modified since the If-Modified-Since header date" do
          it 'sets the Last-Modified header to a valid RFC 2616 date value' do
            get '/', {}, { 'HTTP_IF_MODIFIED_SINCE' => (@last_modified_time + 1).httpdate }
            assert_equal @last_modified_time.httpdate, response['Last-Modified']
          end
