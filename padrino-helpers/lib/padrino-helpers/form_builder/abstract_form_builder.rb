    ...
    module FormBuilder # @private
      class AbstractFormBuilder # @private
        ...
        def label(field, options={})
          ...
          @template.label(field_id(field), options)
        end

        # f.hidden_field :session_id, :value => "45"
        def hidden_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          @template.hidden_field field_name(field), options
        end
        ...
        def text_field(field, options={})
          ...
          @template.text_field field_name(field), options
        end

       def number_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.number_field field_name(field), options
        end

        def telephone_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.telephone_field field_name(field), options
        end
        ...
        def email_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.email_field field_name(field), options
        end

        def search_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.search_field field_name(field), options
        end

        def url_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.url_field field_name(field), options
        end
        ...
        def text_area(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.text_area field_name(field), options
        end
        ...
        def password_field(field, options={})
          options.reverse_merge!(:value => field_value(field), :id => field_id(field))
          options.merge!(:class => field_error(field, options))
          @template.password_field field_name(field), options
        end
        ...
        def select(field, options={})
          ...
          options.merge!(:class => field_error(field, options))
          @template.select field_name(field), options
        end
        ...
        def check_box(field, options={})
          ...
          html  = @template.hidden_field(options[:name] || field_name(field), :value => unchecked_value, :id => nil)
          html << @template.check_box(field_name(field), options)
        end
        ...
        def radio_button(field, options={})
          ...
          options.reverse_merge!(:checked => true) if values_matches_field?(field, options[:value])
          @template.radio_button field_name(field), options
        end
        ...
        def file_field(field, options={})
          ...
          options.merge!(:class => field_error(field, options))
          @template.file_field field_name(field), options
        end

        # f.submit "Update", :class => 'large'
        def submit(caption="Submit", options={})
          @template.submit caption, options
        end

        # f.image_submit "buttons/submit.png", :class => 'large'
        def image_submit(source, options={})
          @template.image_submit source, options
        end
