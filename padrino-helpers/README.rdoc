...
these in constructing a simplified 'form' helper which accepts a block.

  # form '/register' do ... end
...
This will capture the template body passed into the form block and then append the content
...
are three major functions for this category: <tt>tag</tt>, <tt>content</tt> and <tt>input</tt>.

The tag and content are for building arbitrary html tags with a name and specified options. If
the tag contains 'content' within then <tt>content</tt> is used. For example:

  tag(:br, :style => 'clear:both') => <br style="clear:both" />
  content(:p, "demo", :class => 'light') => <p class="light">demo</p>

The input is used to build tags that are related to accepting input from the user:

  input :text, :class => "demo" => <input type='text' class='demo' />
  input :password, :value => "secret", :class => "demo"
...
simple view template:
  ...
  %body
    ...
    %p= image 'padrino.png', :width => '35', :class => 'logo'
...
example of constructing a non-object form would be:
  ...
  - form '/destroy', :class => 'destroy-form', :method => 'delete' do
    = flash_tag:notice)
    - field_set do
      %p
        = label :username, :class => 'first'
        = text_field :username, :value => params[:username]
      %p
        = label :password, :class => 'first'
        = password_field :password, :value => params[:password]
      %p
        = label :strategy
        = select :strategy, :options => ['delete', 'destroy'], :selected => 'delete'
      %p
        = check_box :confirm_delete
    - field_set(:class => 'buttons') do
      = submit "Remove"
