...
describe "Rendering" do
  ...
  context 'for application layout functionality' do
    ...
    should 'be compatible with padrino layout' do
      mock_app do
        layout do
          "this is a <%= yield %>"
        end

        get("/"){ render :erb, "padrino layout" }
      end

      get "/"
      assert_equal "this is a padrino layout", body
    end
  ...
  context 'for application render functionality' do
    ...
    should 'be compatible with padrino render' do
      mock_app do
        get("/"){ render :erb, "<%= 1+2 %>" }
      end
      get "/"
    ...
    should "support passing locals into padrino render" do
      create_layout :application, "layout <%= yield %>"
      create_view :index, "<%= foo %>"
      mock_app do
    ...
    should 'be compatible with padrino views' do
