...
describe 'static' do
  ...
  it 'sets the padrino.static_file env variable if served' do
    env = Rack::MockRequest.env_for("/#{File.basename(__FILE__)}")
    status, headers, body = @app.call(env)
    assert_equal File.expand_path(__FILE__), env['padrino.static_file']
  end
  ...
  it 'handles valid byte ranges correctly' do
    ...
    path = File.dirname(__FILE__) + '/test_helpers.rb'  # currently 16k bytes
    file = File.read(path)
    length = file.length
