...
class FooNotFound < Padrino::Application::NotFound
end
...
describe 'Exception Mappings' do
  ...
  it "sets env['padrino.error'] to the rescued exception" do
    mock_app {
      set :raise_errors, false
      error(FooError) {
        assert env.include?('padrino.error')
        assert env['padrino.error'].kind_of?(FooError)
        'looks good'
      }
      get '/' do
        raise FooError
  ...
  it "never raises Padrino::Application::NotFound beyond the application" do
    mock_app(Padrino::Application) { get('/') { raise Padrino::Application::NotFound }}
    get '/'
    assert_equal 404, status
  end

  it "cascades for subclasses of Padrino::Application::NotFound" do
    mock_app {
      set :raise_errors, true
