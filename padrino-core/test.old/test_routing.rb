...
describe "Routing" do
  ...
  should 'work correctly with padrino redirects' do
    mock_app do
  ...
  should 'render padrino NotFound page' do
    mock_app { set :environment, :development }
    get "/"
    assert_equal 404, status
    assert_match %r{(Padrino doesn&rsquo;t know this ditty.|<h1>Not Found</h1>)}, body
  end

  should 'render a custom NotFound page' do
    mock_app do
      error(Padrino::NotFound) { "not found" }
    end
    get "/"
    assert_equal 404, status
