  ...
  module Reloader
    ...
    class << self
      ...
      def remove_constant(const)
        return if Padrino::Reloader.exclude_constants.any? { |base| (const.to_s =~ %r{^#{Regexp.escape(base)}}) } &&
                 !Padrino::Reloader.include_constants.any? { |base| (const.to_s =~ %r{^#{Regexp.escape(base)}}) }
        begin
