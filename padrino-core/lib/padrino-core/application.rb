module Padrino
  ...
  class Application < Sinatra::Application
    ...
    private
      ...
      # 
      def render(data=nil, options={}, locals={}, &block)
        ...
        # If data is an hash we convert to json
        return data.to_json if data.is_a?(Hash)
        # If an data is a string probably is a path so we try to resolve them
        data   = data.to_sym
        engine = resolve_template_engine(data)
      ...
      def resolve_template_engine(template_path)
        ...
        raise "Template path '#{template_path}' could not be located in: #{self.options.views}" unless template_file
