...
module Padrino
    ...
    module Routing
        ...
        CONTENT_TYPE_ALIASES = { htm: :html }.freeze unless defined?(CONTENT_TYPE_ALIASES)
        # Defines the available route priorities supporting route deferrals.
        ROUTE_PRIORITY = { high: 0, normal: 1, low: 2 }.freeze unless defined?(ROUTE_PRIORITY)
        ...
        module ClassMethods
            ...
            def put(path, *args, &block)
                route 'PUT',     path, *args, &block
            end

            def post(path, *args, &block)
                route 'POST',    path, *args, &block
            end

            def delete(path, *args, &block)
                route 'DELETE',  path, *args, &block
            end
            ...
            def options(path, *args, &block)
                route 'OPTIONS', path, *args, &block
            end

            def patch(path, *args, &block)
                route 'PATCH',   path, *args, &block
            end

            def link(path, *args, &block)
                route 'LINK',    path, *args, &block
            end

            def unlink(path, *args, &block)
                route 'UNLINK',  path, *args, &block
            end
            ...
            def route(verb, path, *args, &block)
                ...
                if protect_from_csrf && (report_csrf_failure || allow_disabled_csrf)
                    unless route_options.key?(:csrf_protection)
                ...
                (priority = ROUTE_PRIORITY[priority_name]) || raise("Priority #{priority_name} not recognized, try #{ROUTE_PRIORITY.keys.join(', ')}")
            ...
            #
            def parse_route(path, options, _verb)
        ...
        module InstanceMethods
            ...
            def dispatch!
                ...
                filter! :before if boom.is_a? ::Sinatra::NotFound
