      ...
      private
        ...
        def render(engine, data=nil, options={}, locals={}, &block)
          ...
          if (options[:layout].nil? || options[:layout] == true) && !settings.templates.has_key?(:layout)
            ...
            logger.debug "Resolving layout #{settings.root}/views#{options[:layout]}" if defined?(logger) && options[:layout].present?
          elsif options[:layout].present?
            options[:layout] = settings.fetch_layout_path(options[:layout])
            logger.debug "Resolving layout #{settings.root}/views#{options[:layout]}" if defined?(logger)
          end
