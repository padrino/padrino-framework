...
pre[bash]. sample-blog $ heroku create --stack bamboo-ree-1.8.7
...
Currently Padrino defaults to *SQLite* but Heroku only supports *PostgreSQL*, so we'll need to add @pg@ as a
dependency.
...
pre[bash]. sample_blog $ heroku rake ar:migrate
sample_blog $ heroku rake seed

You'll see something like:

<pre lang="bash">sample_blog $ heroku rake ar:migrate
