...
* FIX {#624}[] potential issue with libxml-ruby [Thanks {@farcaller}[]]
* FIX {#618}[] allow to map routes using "index" reserved word
* FIX {#633}[] RACK_ENV now is respected also using bundled server
* FIX rendering error for partials when they start with a forward slash [Thanks {@philly-mac}[]]
* FIX all our sources files now are vim friendly
* FIX preserve the original options when resolving a template [Thanks {@Xylakant}[]]
* FIX bad path in shoulda test generator [Thanks {@futurechimp}[]]
...
* FIX {#639}[] prevent XSS from admin login [Thanks {@eikes}[]]
