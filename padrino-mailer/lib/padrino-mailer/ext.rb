    ...
    class Message # @private
        ...
        def add_resolved_part(attributes = {}, &block)
            ...
            if block_given? || value
                instance_variable_set "@#{variable}", part(content_type: content_type,
                                                           body: value,
                                                           part_block: block)
                add_multipart_alternate_header if send(variable)
            else
        ...
        def content_type_with_symbol(value = nil)
            if value.is_a?(Symbol)
                value = begin
                          Padrino::Mailer::Mime::MIME_TYPES.find { |_k, v| v == value }[0]
                      rescue
                          value
                      end
            end
