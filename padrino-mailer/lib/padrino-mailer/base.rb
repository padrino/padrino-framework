    ...
    module Mailer
        ...
        class Base
            ...
            def email(name, &block)
                raise "The email '#{name}' is already defined" if messages[name]
                messages[name] = proc do |*attrs|
                    message = app.settings._padrino_mailer::Message.new(app)
                    message.mailer_name = mailer_name
                    message.message_name = name
                    message.defaults = defaults if defaults.any?
