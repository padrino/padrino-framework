...
class TestMigrationGenerator < Test::Unit::TestCase
  ...
  context 'the migration generator' do
    should "fail outside app root" do
      output = silence_logger { Padrino::Generators::Migration.start(['add_email_to_users', '-r=/tmp']) }
    ...
    should "fail if we don't use an adapter" do
      ...
      assert_raise(SystemExit) { silence_logger { Padrino::Generators::Migration.start(['AddEmailToUsers', '-r=/tmp/sample_project']) } }
    end
    ...
    should "generate migration inside app root" do
      ...
      response_success = silence_logger { Padrino::Generators::Migration.start(['AddEmailToUsers', '-r=/tmp/sample_project']) }
    ...
    should "generate migration inside app root with lowercase migration argument" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=activerecord']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['add_email_to_users', '-r=/tmp/sample_project']) }
      migration_file_path = "/tmp/sample_project/db/migrate/001_add_email_to_users.rb"
    ...
    should "generate migration inside app root with singular table" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=activerecord']) }
      silence_logger { Padrino::Generators::Migration.start(['add_email_to_user', "email:string", '-r=/tmp/sample_project']) }
    ...
    should "properly calculate version number" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=sequel']) }
      silence_logger { Padrino::Generators::Migration.start(['add_email_to_person', "email:string", '-r=/tmp/sample_project']) }
      silence_logger { Padrino::Generators::Migration.start(['add_name_to_person', "email:string", '-r=/tmp/sample_project']) }
      silence_logger { Padrino::Generators::Migration.start(['add_age_to_user', "email:string", '-r=/tmp/sample_project']) }
  ...
  context 'the migration generator for activerecord' do
    should "generate migration for generic needs" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=activerecord']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['ModifyUserFields', '-r=/tmp/sample_project']) }
    ...
    should "generate migration for adding columns" do
      ...
      response_success = silence_logger { Padrino::Generators::Migration.start(migration_params) }
      migration_file_path = "/tmp/sample_project/db/migrate/001_add_email_to_users.rb"
      assert_match_in_file(/class AddEmailToUsers/m, migration_file_path)
  ...
  context 'the migration generator for datamapper' do
    should "generate migration for generic needs" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=datamapper']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['ModifyUserFields', '-r=/tmp/sample_project']) }
      migration_file_path = "/tmp/sample_project/db/migrate/001_modify_user_fields.rb"
    ...
    should "properly version migration files" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=datamapper']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['ModifyUserFields', '-r=/tmp/sample_project']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['ModifyUserFields2', '-r=/tmp/sample_project']) }
      response_success = silence_logger { Padrino::Generators::Migration.start(['ModifyUserFields3', '-r=/tmp/sample_project']) }
  ...
  context "the migration destroy option" do
    ...
    should "destroy the migration files" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon', '-d=sequel']) }
      migration_params = ['RemoveEmailFromUsers', "email:string", "age:integer", '-r=/tmp/sample_project']
      silence_logger { Padrino::Generators::Migration.start(migration_params) }
      silence_logger { Padrino::Generators::Migration.start(['RemoveEmailFromUsers', '-r=/tmp/sample_project','-d']) }
    ...
    should "destroy the migration file regardless of number" do
      ...
      silence_logger { Padrino::Generators::Migration.start(migration_param2) }
      silence_logger { Padrino::Generators::Migration.start(migration_params) }
      silence_logger { Padrino::Generators::Migration.start(['RemoveEmailFromUsers', '-r=/tmp/sample_project','-d']) }
