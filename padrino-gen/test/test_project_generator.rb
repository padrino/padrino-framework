...
class TestProjectGenerator < Test::Unit::TestCase
  ...
  context 'the project generator' do
    should "allow simple generator to run and create base_app with no options" do
      assert_nothing_raised { silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none']) } }
    ...
    should "not create models folder if no orm is chosen" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '--orm=none']) }
      assert_no_dir_exists('/tmp/sample_project/app/models')
    end

    should "not create tests folder if no test framework is chosen" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '--test=none']) }
      assert_no_dir_exists('/tmp/sample_project/test')
    end

    should "place app specific names into correct files" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none']) }
    ...
    should "create components file containing options chosen" do
      ...
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', *component_options]) }
      components_chosen = YAML.load_file('/tmp/sample_project/.components')
    ...
    should "output to log components being applied" do
      component_options = ['--orm=datamapper', '--test=riot', '--mock=mocha', '--script=prototype', '--renderer=erb','--stylesheet=less']
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', *component_options]) }
  ...
  context "a generator for mock component" do
    should "properly generate for rr and riot" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--mock=rr', '--test=riot', '--script=none']) }
    ...
    should "properly generater for rr and bacon" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--mock=rr', '--test=bacon', '--script=none']) }
      assert_match /Applying.*?rr.*?mock/, buffer
      assert_match_in_file(/gem 'rr'/, '/tmp/sample_project/Gemfile')
    ...
    should "properly generate for mocha and rspec" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--mock=mocha', '--script=none']) }
      assert_match /Applying.*?mocha.*?mock/, buffer
    ...
    should "properly generate for rr and rspec" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--mock=rr', '--script=none']) }
      assert_match /Applying.*?rr.*?mock/, buffer
      assert_match_in_file(/gem 'rr'/, '/tmp/sample_project/Gemfile')
  ...
  context "the generator for orm components" do
    should "properly generate for sequel" do
      ...
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--orm=sequel', '--script=none']) }
    ...
    should "properly generate for activerecord" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--orm=activerecord', '--script=none']) }
    ...
    should "properly generate default for datamapper" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--orm=datamapper', '--script=none']) }
      assert_match /Applying.*?datamapper.*?orm/, buffer
    ...
    should "properly generate for mongomapper" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--orm=mongomapper', '--script=none']) }
    ...
    should "properly generate for couchrest" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--orm=couchrest', '--script=none']) }
  ...
  context "the generator for renderer component" do
    should "properly generate default for erb" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--renderer=erb', '--script=none']) }
      assert_match /Applying.*?erb.*?renderer/, buffer
    end

    should "properly generate for haml" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--renderer=haml','--script=none']) }
  ...
  context "the generator for script component" do
    should "properly generate for jquery" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=jquery']) }
    ...
    should "properly generate for mootools" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=mootools']) }
    ...
    should "properly generate for prototype" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=prototype']) }
      assert_match /Applying.*?prototype.*?script/, buffer
    ...
    should "properly generate for rightjs" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=rightjs']) }
    ...
    should "properly generate for ext-core" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=extcore']) }
  ...
  context "the generator for test component" do
    should "properly default generate for bacon" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=bacon', '--script=none']) }
    ...
    should "properly generate for riot" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=riot', '--script=none']) }
      assert_match /Applying.*?riot.*?test/, buffer
    ...
    should "properly generate for rspec" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=rspec', '--script=none']) }
    ...
    should "properly generate for shoulda" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=shoulda', '--script=none']) }
    ...
    should "properly generate for testspec" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=testspec', '--script=none']) }
    ...
    should "properly generate for cucumber" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--test=cucumber', '--script=none']) }
  ...
  context "the generator for stylesheet component" do
    should "properly generate for sass" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--renderer=haml','--script=none','--stylesheet=sass']) }
      assert_match_in_file(/gem 'haml'/, '/tmp/sample_project/Gemfile')
    ...
    should "properly generate for less" do
      buffer = silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--renderer=haml','--script=none','--stylesheet=less']) }
