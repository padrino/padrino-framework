...
class TestMailerGenerator < Test::Unit::TestCase
  ...
  context 'the mailer generator' do
    should "fail outside app root" do
      output = silence_logger { Padrino::Generators::Mailer.start(['demo', '-r=/tmp']) }
    ...
    should "support generating a new mailer extended from base" do
      ...
      silence_logger { Padrino::Generators::Mailer.start(['demo', '-r=/tmp/sample_project']) }
    ...
    should "support generating a new mailer extended from base with long name" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon']) }
      silence_logger { Padrino::Generators::Mailer.start(['user_notice', '-r=/tmp/sample_project']) }
  ...
  context "the mailer destroy option" do
    ...
    should "destroy mailer file" do
      silence_logger { Padrino::Generators::Project.start(['sample_project', '--root=/tmp', '--script=none', '-t=bacon']) }
      silence_logger { Padrino::Generators::Mailer.start(['demo', '-r=/tmp/sample_project']) }
      silence_logger { Padrino::Generators::Mailer.start(['demo', '-r=/tmp/sample_project','-d']) }
