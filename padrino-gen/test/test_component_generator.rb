...
describe "ComponentGenerator" do
  ...
  describe 'the controller generator' do
    it 'should fail outside app root' do
      out, _ = capture_io { generate(:component, 'demo', "-r=#{@apptmp}") }
  ...
  describe "add components" do
    it 'should properly generate default' do
      ...
      out, _ = capture_io { generate(:component, '--orm=activerecord', "-r=#{@apptmp}/sample_project") }
    ...
    it 'should properly generate with adapter' do
      capture_io { generate(:project, 'sample_project', "--root=#{@apptmp}") }
      out, _ = capture_io { generate(:component, '--orm=sequel', '--adapter=postgres', "-r=#{@apptmp}/sample_project") }
    ...
    it 'should enable @app_name value' do
      capture_io { generate(:project, 'sample_project', "--root=#{@apptmp}") }
      _, _ = capture_io { generate_with_parts(:component, '--test=cucumber', "-r=#{@apptmp}/sample_project", :apps => "app") }
    ...
    it 'should generate component in specified app' do
      ...
      _, _ = capture_io { generate_with_parts(:component, '--test=cucumber', "--app=compo", "-r=#{@apptmp}/sample_project", :apps => "compo") }
    ...
    it 'should not generate component in specified app if the app does not exist' do
      capture_io { generate(:project, 'sample_project', "--root=#{@apptmp}") }
      out, _ = capture_io { generate_with_parts(:component, '--test=cucumber', "--app=compo", "-r=#{@apptmp}/sample_project", :apps => "compo") }
  ...
  describe "specified of same the component" do
    it 'should does not change' do
      ...
      out, _ = capture_io { generate(:component, '--script=jquery', "-r=#{@apptmp}/sample_project") }
  ...
  describe "component changes" do
    it 'should when allow changes, will be applied' do
      ...
      out, _ = capture_io { generate(:component, '--renderer=haml', "-r=#{@apptmp}/sample_project") }
