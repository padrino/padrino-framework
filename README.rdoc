...
MultiApp:: unlike other ruby framework Padrino as Django do is designed from principe for handle mutliple apps at the same time.
Routing:: Full url named route support to avoid hardcoding route url paths (<tt>map</tt>, <tt>url_for</tt>)
Tag Helpers:: tag helpers like: <tt>tag</tt>, <tt>content_tag</tt>, <tt>input_tag</tt>
Asset Helpers:: like: <tt>link_to</tt>, <tt>image_tag</tt>, <tt>javascript_include_tag</tt>, ...
Form Helpers:: with builders support like: <tt>form_tag</tt>, <tt>form_for</tt>, <tt>field_set_tag</tt>, <tt>text_field</tt>, ...
...
Admin:: an ajax admin that your records in sortable grids, tree, windows ... as a desktop app can do.
